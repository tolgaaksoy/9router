...
export async function createZedNativeAuthData(config = {}, options = {}) {
...
export function decryptZedAccessToken(encryptedAccessToken, privateKeyVerifier) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Failed to decrypt Zed access token: ${oaepError.message}`);
    }
  }
}
...
export function resolveZedOrganizationId(credentials, userInfo = null) {
  ...
  const org = (userInfo?.organizations || []).find((item) => item?.is_personal) || userInfo?.organizations?.[0];
...
export async function fetchZedLlmToken(credentials, options = {}) {
  ...
  if (!options.forceRefresh && cached?.expiresAt > Date.now()) return cached.token;
...
function normalizeZedModelId(id) {
  ...
  if (typeof id === "object" && typeof id[0] === "string") return id[0];
...
export async function resolveZedModels(credentials, options = {}) {
  ...
  if (!options.forceRefresh && cached?.expiresAt > Date.now()) return cached;
  ...
  const promise = (async () => {
    ...
    const models = rawModels.map(mapZedModel).filter(Boolean).filter((model) => !model.isDisabled);
    ...
    const entry = {
      ...
      recommendedModels: (data?.recommended_models || data?.recommendedModels || []).map(normalizeZedModelId).filter(Boolean),
    };
